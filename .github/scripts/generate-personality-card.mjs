...
function generateSvg() {
  ...
  <text x="18" y="38" font-size="24" font-weight="800" fill="#ffffff">Personality Snapshot</text>
  <text x="18" y="59" font-size="15" fill="#c4b5fd">Giselle \u00B7 Architect \u00B7 INTJ-T</text>
  <text x="18" y="77" font-size="13" font-weight="700" fill="#e0e7ff">90% Judging \u00B7 89% Intuitive</text>
  <text x="18" y="${TRAITS_Y - 8}" font-size="12" font-weight="700" fill="#6b7280" letter-spacing="1.4">TRAIT BREAKDOWN</text>`;

  for (let i = 0; i < TRAITS.length; i++) {
    const ty = TRAITS_Y + i * TRAIT_STEP;
    const fill = Math.round(BAR_W * TRAITS[i].pct / 100);
    body += `
  <text x="18" y="${ty + 13}" font-size="15" font-weight="700" fill="#374151">${esc(TRAITS[i].name)}</text>
  <text x="98" y="${ty + 13}" font-size="13" fill="#9ca3af">${esc(TRAITS[i].sub)}</text>
  <rect x="18" y="${ty + 19}" width="${BAR_W}" height="10" rx="5" fill="#e9ecf0"/>
  <rect x="18" y="${ty + 19}" width="${fill}" height="10" rx="5" fill="${TRAITS[i].color}"/>
  <text x="${18 + BAR_W + 10}" y="${ty + 28}" font-size="14" font-weight="700" fill="${TRAITS[i].color}">${TRAITS[i].pct}%</text>`;
  }
  body += `
  <text x="${W / 2}" y="${H - 7}" text-anchor="middle" font-size="10" fill="#9ca3af">updated by GitHub Actions</text>`;
